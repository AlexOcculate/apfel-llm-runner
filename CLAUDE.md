...
- Version: `1.8.2` (source of truth: `.version`)
- Tests: 989 unit + 454 integration
...
swift run apfel-tests          # unit tests only (989 tests)
make preflight                 # full release qualification (unit + integration + policy checks)
```

`make test` builds the release binary, runs all 989 unit tests, starts test servers, runs all 454 integration tests, and cleans up. This is the single command for development.
...
| Tests | `Tests/apfelTests/` (989 unit), `Tests/integration/` (454 integration) |
